...
class BrandBehaviorAttributeRepo extends OmedaRepo {
   ...
   *
   * @param {object} params
   * @param {object[]} params.attributes The attributes to upsert.
   */
  async upsert(params = {}) {
    const { attributes } = await validateAsync(Joi.object({
      attributes: Joi.array().items(Joi.object().required()).required(),
    }).required(), params);
    ...
    const now = new Date();
    const ids = [];
    const operations = attributes.map((attribute) => {
      const { Id } = attribute;
      ids.push(Id);
      const filter = { brand, 'data.Id': Id };
      const update = {
        $setOnInsert: { brand, createdAt: now },
        $set: { data: attribute, updatedAt: now },
      };
      return { updateOne: { filter, update, upsert: true } };
    });

    // delete any remaining attributes that weren't included in this dataset
    operations.push({
